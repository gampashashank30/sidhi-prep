...
function ResetPasswordForm() {
  ...
  // Validate the token on mount via the set-password GET endpoint (reused)
  useEffect(() => {
    ...
    return (
      <>
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '1.25rem', fontWeight: 700, color: '#0F172A', marginBottom: '0.5rem' }}>Invalid or expired link</h2>
          <p style={{ fontSize: '0.875rem', color: '#64748B', marginBottom: '1.5rem', lineHeight: 1.6 }}>This password reset link is invalid or has expired. Please request a new one.</p>
        </div>
        <Link href="/forgot-password" style={{ display: 'block', textAlign: 'center', padding: '0.75rem', background: 'linear-gradient(135deg, #1B5EA7 0%, #14B89A 100%)', color: '#fff', borderRadius: '0.625rem', fontWeight: 700, textDecoration: 'none', fontSize: '0.9375rem' }}>
  ...
  if (tokenValid === null) {
    return <div style={{ textAlign: 'center', color: '#94A3B8', padding: '2rem 0' }}>Validating link…</div>;
  }
  ...
  if (success) {
    return (
      <div style={{ textAlign: 'center' }}>
        ...
        <h2 style={{ fontSize: '1.25rem', fontWeight: 700, color: '#0F172A', marginBottom: '0.5rem' }}>Password updated!</h2>
        <p style={{ fontSize: '0.875rem', color: '#64748B' }}>Redirecting you to sign in…</p>
      </div>
    );
  }
  ...
  return (
    <>
      <h2 style={{ fontSize: '1.25rem', fontWeight: 700, color: '#0F172A', marginBottom: '0.25rem' }}>Set new password</h2>
      <p style={{ fontSize: '0.8125rem', color: '#94A3B8', marginBottom: '1.5rem' }}>Choose a strong password with at least 8 characters.</p>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {(['password', 'confirm'] as const).map((field) => (
          <div key={field}>
            <label htmlFor={`reset-${field}`} style={{ display: 'block', fontSize: '0.8125rem', fontWeight: 600, color: '#374151', marginBottom: '0.375rem' }}>
